  ...
  if (user) {
    ...
    authButtons.innerHTML = `
      <div class="user-info" style="display:flex;align-items:center;gap:8px">
        <a href="../products/dashboard.html"    class="btn btn-outline btn-sm">
          <i class="fas fa-store"></i> Listings
        </a>
        <a href="../chat/messaging.html"    class="btn btn-outline btn-sm" style="position:relative">
          <i class="fas fa-comment-dots"></i> Messages
          ${msgCount ? `<span class="nav-badge" style="position:static;margin-left:2px">${msgCount > 9 ? '9+' : msgCount}</span>` : ''}
        </a>
        <a href="../notifications/notifications.html" class="btn btn-outline btn-sm" style="position:relative">
          <i class="fas fa-bell"></i>
          ${notifCount ? `<span class="nav-badge" style="position:static;margin-left:2px">${notifCount > 9 ? '9+' : notifCount}</span>` : ''}
        </a>
        <a href="../profile/menu.html" class="nav-avatar" style="cursor:pointer">${avatarHtml}</a>
      </div>
    `;
  } else {
    authButtons.innerHTML = `
