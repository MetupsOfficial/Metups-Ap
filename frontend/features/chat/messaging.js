...
import { supabaseClient, SUPABASE_URL, SUPABASE_ANON_KEY } from '/assets/js/supabase.js';

const FUNCTIONS_URL = `${SUPABASE_URL}/functions/v1`;
import { checkAuth, handleError, getCurrentUser } from '/assets/js/utils.js';
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      try {
        ...
        const ext      = mediaRecorder.mimeType?.includes('ogg')  ? 'ogg'
                       : mediaRecorder.mimeType?.includes('mp4')  ? 'mp4'
                       : 'webm';
        const fileName = `conversations/${conversationId}/voice_${Date.now()}.${ext}`;
        ...
        await sendMessage(conversationId, '🎤 Voice message', 'voice', signedData.signedUrl);
      } catch (err) {
    ...
    mediaRecorder.start(100); // 100ms timeslice — prevents buffer cutoff on long recordings
    isRecording = true;
    onStateChange?.(true);
